...
- If deploying to Kinetic, set a valid market address in `packages/foundry/script/DeployFlarePublicGoods.s.sol` by replacing `kineticMarket`.
- Run:
```bash
...
# Flare Public Goods PLSA

Welcome to the Flare Public Goods Prize-Linked Savings Account! This is a decentralized application where users can deposit USDC to earn yield from the Morpho Blue protocol. The accumulated yield is then awarded as a prize to a random depositor in a weekly draw. It's a no-loss savings game: you can always withdraw your principal, and you get a chance to win the prize pool!
...
3.  **Run the deployment script**:
    ...
    forge script script/DeployFlarePublicGoods.s.sol:DeployFlarePublicGoods --rpc-url ${SEPOLIA_RPC_URL} --broadcast --verify
    ```

4.  **Copy the Deployed Address**: The script will output the address of the newly deployed `FlarePublicGoods` contract. Copy this address.

5.  **Update the Frontend**:
    *   Open `packages/nextjs/src/contracts/FlarePublicGoods.ts`.
...
```
forge script packages/foundry/script/DeployFlarePublicGoods.s.sol \
  --rpc-url sepolia \
...
3. After deployment, copy the deployed address and update the frontend config:
    ...
    - Edit `src/contracts/FlarePublicGoods.ts` and set the Coston2 entry under `CONTRACTS[114].pumpkinSpiceLatte` to your new address.
...
forge verify-contract \
  ...
  packages/foundry/src/FlarePublicGoods.sol:FlarePublicGoods
```

```
forge verify-contract 0x3ecc78c6fea14565affd607bd35b5b8e6dc39778 FlarePublicGoods --etherscan-api-key $TENDERLY_ACCESS_KEY --verifier-url https://virtual.mainnet.us-east.rpc.tenderly.co/420b1805-6a91-4b32-b1c2-d37896a360cb/verify/etherscan --watch
```
...
=== Flare Contract Addresses ===
  ...
  === Deployment Complete ===
  FlarePublicGoods deployed: 0xc73ECA128c225564C4151fAa723Ae0527FDB8516
...
forge verify-contract --chain 114 --verifier blockscout \
  ...
  0x85b84761d95cc3C4832D941363DCFee17e3d33a4 packages/foundry/src/FlarePublicGoods.sol:FlarePublicGoods
```
...
=== Flare Contract Addresses ===
  ...
  For mainnet, verify these addresses match the target network
  === Deployment Complete ===
  FlarePublicGoods deployed: 0x85b84761d95cc3C4832D941363DCFee17e3d33a4
...
=== Flare Contract Addresses ===
  ...
  For mainnet, verify these addresses match the target network
  === Deployment Complete ===
  FlarePublicGoods deployed: 0xf94d594A61358761FAcDCe77E5Ff4303dad12a49
  Kinetic or Morpho4626Adapter: 0x351957C2D6043B258CCC5f4da5a60a2E132Cf158
  Random Number Provider: FlareSecureRandomAdapter (Secure VRF)
  RNG Address: 0xB21Fb6D1399D08147F192be583340f5D5D0aEd7A
...
=== Flare Network Info ===
  ...
  == Logs ==
  === Deployment Complete ===
  FlarePublicGoods deployed: 0xdB9212aF6019C137AC321d36DCEB3e174D933b37
