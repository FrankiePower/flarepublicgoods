...
// =======
	...
	useEffect(() => {
		...
		const appendItems = (newItems: FundingAllocation[]) => {
			if (cancelled || newItems.length === 0) return;
			setAllocations(prev => {
				const map = new Map<string, FundingAllocation>();
				for (const it of prev) map.set(`${it.txHash ?? ''}:${it.blockNumber.toString()}:${it.recipient}`, it);
				for (const it of newItems) map.set(`${it.txHash ?? ''}:${it.blockNumber.toString()}:${it.recipient}`, it);
...
// >>>>>>> main
  ...
  //         toBlock: 'latest',
  //       });
  //       const items: FundingAllocation[] = logs.map((log) => ({
  //         blockNumber: log.blockNumber ?? 0n,
  ...
  const yourTotalReceived = useMemo(() => {
    if (!address) return 0n;
    return allocations.reduce(
      (acc, w) =>
        w.recipient.toLowerCase() === address.toLowerCase() ? acc + w.amount : acc,
      0n
    );
  }, [allocations, address]);
    ...
    <Card className='border-0 shadow-none bg-transparent'>
      <CardHeader className='pb-3'>
        <CardTitle className='flex items-center gap-2 text-xl'>
          <span className='text-2xl'>📜</span> Funding History
        </CardTitle>
        <CardDescription className='text-sm'>
          Recent funding allocations and your total contribution impact.
        </CardDescription>
        ...
        <div className='bg-[#f5f2f0] rounded-lg p-4'>
          <div className='flex justify-between items-center'>
            ...
            <p className='font-bold text-lg'>
              {formatUnits(yourTotalReceived, 6)} USDC
            </p>
          </div>
        </div>
        <div>
          <p className='text-sm text-muted-foreground mb-3 flex items-center gap-2'>
            <span className='text-lg'>🎯</span> Recent allocations
          </p>
          <div className='border rounded-lg bg-white'>
            {loading ? (
              ...
              <div className='p-4 text-sm text-muted-foreground'>
                No funding allocations yet. 🌱
              </div>
            ) : (
              <div className='max-h-48 overflow-y-auto'>
                <ul className='divide-y'>
                  {allocations.map((w, idx) => (
                    <li
                      key={`${w.txHash}-${idx}`}
                    ...
                    >
                      <div className='min-w-0'>
                        <a
                          href={getAddressExplorerUrl(chainId, w.recipient)}
                          target='_blank'
                          rel='noreferrer'
