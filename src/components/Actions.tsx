...
const Actions = () => {
  ...
  const contractAddress = isSupportedNetwork
    ? CONTRACTS[chain.id as keyof typeof CONTRACTS].flarePublicGoods
    : flarePublicGoodsAddress;

  // Prefer the on-chain configured asset from FPG to avoid mismatches; fall back to mapping
  ...
  const { writeContract: approve, isPending: isApproving } = useWriteContract({
    onSuccess: (hash: `0x${string}`) => {
      ...
      console.log('[FPG] Approval tx submitted:', hash);
      toast({
  ...
  const { writeContract: deposit, isPending: isDepositing } = useWriteContract({
    onSuccess: (hash: `0x${string}`) => {
      setDepositHash(hash);
      // eslint-disable-next-line no-console
      console.log('[FPG] Deposit tx submitted:', hash);
      toast({
  ...
  const { writeContract: revoke, isPending: isRevoking } = useWriteContract({
    onSuccess: (hash: `0x${string}`) => {
      setRevokeHash(hash);
      // eslint-disable-next-line no-console
      console.log('[FPG] Revoke approval tx submitted:', hash);
      toast({
  ...
  useEffect(() => {
    if (approvalError) {
      // eslint-disable-next-line no-console
      console.error('[FPG] Approval receipt error:', approvalError);
      toast({
  ...
  useEffect(() => {
    if (isApprovalConfirmed) {
      // eslint-disable-next-line no-console
      console.log('[FPG] Approval confirmed:', {
        approvalHash,
        approvalStatus,
      });
      refetchAllowance();
  ...
  useEffect(() => {
    if (depositError) {
      // eslint-disable-next-line no-console
      console.error('[FPG] Deposit receipt error:', depositError);
      toast({
        title: 'Deposit Error',
  ...
  useEffect(() => {
    if (revokeError) {
      // eslint-disable-next-line no-console
      console.error('[FPG] Revoke receipt error:', revokeError);
      toast({
        title: 'Revoke Error',
  ...
  useEffect(() => {
    if (isDepositConfirmed) {
      // eslint-disable-next-line no-console
      console.log('[FPG] Deposit confirmed:', { depositHash, depositStatus });
      toast({
